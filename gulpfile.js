...
function browsersync() {
    browserSync.init({
        ...
        port: 3000
        // server: {
        //     baseDir: 'www' // здесь указываем корневую папку для локального сервера
        // }
    });
}
...
function watching() { //следит за обновленем файла
    ...
    watch(['app/style.css']).on('change', browserSync.reload);
    watch(['www/style.css']).on('change', browserSync.reload);
    ...
    watch(['www/index.html']).on('change', browserSync.reload); //просит сервер обновить страницу полностью
    watch(['www/news.php']).on('change', browserSync.reload); //просит сервер обновить страницу полностью
}
